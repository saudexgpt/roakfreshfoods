...
class ReportsController extends Controller
{
    ...
    public function productsInStockGraph(Request $request)
    {
        ...
        $items = Item::orderBy('name')->orderBy('category_id')->get();
        foreach ($items as $item) {
            $item_in_stock = ItemStockSubBatch::with('item')
                ->groupBy('item_id')
                ->where(['warehouse_id' => $warehouse_id, 'item_id' => $item->id])
                ->select('*', \DB::raw('SUM(quantity) as quantity'), \DB::raw('SUM(in_transit) as in_transit'), \DB::raw('SUM(balance) as total_balance'), \DB::raw('SUM(supplied) as total_supplied'))
            ...
            $item_name = $item->name;
            $categories[] = $item_name . ' (' . $item->basic_unit . ')';
            ...
            $quantity[] = [
                'name' => $item_name,
                'y' => (float) $total_stocked,
            ];
            $in_transit[] = [
                'name' => $item_name,
                'y' => (float) $item_in_transit,
            ];

            $balance[] = [
                'name' => $item_name,
                'y' => (float) $total_balance,
            ];
            $supplied[] = [
                'name' => $item_name,
                'y' => (float) $total_supplied,
            ];
        }
        $series = [
            [
    ...
    public function salesByItems(Request $request)
    {
        ...
        $product_categories = Category::with('items')->orderBy('name')->get();
        foreach ($product_categories as $product_category) {
            $items = $product_category->items;
            $items_ids_array = $items->pluck('id');
            $sales = InvoiceItem::with('item')->groupBy('item_id')
                ->where('warehouse_id', $warehouse_id)
                ->whereIn('item_id', $items_ids_array)
            ...
            foreach ($sales as $sale) {
                ...
                $drilldown_data[] = [$item_name, $sale->total_amount];
            }
            $data[] = [
                'name' => $name,
                'y' => (int) $total_amount,
        ...
        $extra_title = ' from ' . date('M d, Y', strtotime($date_from)) . ' to ' . date('M d, Y', strtotime($date_to));
        $subtitle = /*strtoupper($panel . 'ly Report ') . */ 'For ' . $warehouse->name . $extra_title;
        return response()->json(
            [
    ...
    public function outbounds(Request $request)
    {
        ...
        $warehouse_id = $request->warehouse_id;
        $status = (isset($request->status)) ? $request->status : 'delivered';
        $date_from = Carbon::now()->startOfMonth();
        $date_to = Carbon::now()->endOfMonth();
        $panel = 'month';
        if (isset($request->from, $request->to, $request->panel)) {
        ...
        $invoice_items = InvoiceItem::with('item', 'invoice')
            ->where('warehouse_id', $warehouse_id)
        ...
        foreach ($invoice_items as $invoice_item) {
            ...
            $unit = $invoice_item->item->basic_unit;

            $outbounds[] = [
                'invoice_no' => $invoice_item->invoice->invoice_number,
                'customer' => ($invoice_item->invoice->customer) ? $invoice_item->invoice->customer->user->name : 'Anonymous',
                'product' => ($invoice_item->item) ? $invoice_item->item->name : '',
                'amount' => $invoice_item->amount,
                'quantity' => $invoice_item->quantity . ' ' . $unit, // initially set to zero
        ...
        $title = 'Sales Report from ' . date('M d, Y', strtotime($date_from)) . ' to ' . date('M d, Y', strtotime($date_to));
        ini_set('memory_limit', '128M');
        return response()->json(compact('outbounds', 'columns', 'title'));
    }
    ...
    public function fetchBinCard(Request $request)
    {
        ...
        if (isset($request->batch_no) && $request->batch_no != '') {
            ...
            list($total_stock_till_date, $inbounds, $outbounds, $expired_products) = $this->getProductTransactionBasedOnBatchNo($item_id, $date_from, $date_to, $warehouse_id, $products_in_stock_ids);

        } else {
            list($total_stock_till_date, $previous_outbound, $inbounds, $outbounds, $expired_products) = $this->getProductTransaction($item_id, $date_from, $date_to, $warehouse_id);
        }
        ...
        $brought_forward = (float) $quantity_in_stock - (float) $quantity_supplied;
        if ($inbounds->isNotEmpty()) {
            foreach ($inbounds as $inbound) {
                ...
                $bincards[] = [
                    ...
                    'quantity_transacted' => (float) $inbound->total_quantity,
                    'in' => (float) $inbound->total_quantity,
                    'out' => '',
        ...
        if ($outbounds->isNotEmpty()) {
            foreach ($outbounds as $outbound) {
                ...
                $bincards[] = [
                    ...
                    'invoice_no' => $outbound->invoice->invoice_number,
                    'waybill_grn' => '-', // $outbound->waybill->waybill_no,
                    'quantity_transacted' => (float) $outbound->total_quantity_supplied,
                    'in' => '',
                    'out' => (float) $outbound->total_quantity_supplied,
                    'balance' => 0, // initially set to zero
                    'packaging' => $item->package_type,
                    'physical_quantity' => '',
                    'sign' => '',
    ...
    private function getProductTransactionBasedOnBatchNo($item_id, $date_from, $date_to, $warehouse_id, $products_in_stock_ids = [])
    {
        ...
        $total_stock_till_date = ItemStockSubBatch::groupBy('item_id')
            ...
            ->first();
        $previous_outbound = InvoiceItem::groupBy('item_id')
        ...
        $outbounds = InvoiceItem::groupBy(['item_id'])
            ->whereIn('item_stock_sub_batch_id', $products_in_stock_ids)
            ->where('warehouse_id', $warehouse_id)
            ->where('item_id', $item_id)
            ->where('created_at', '>=', $date_from)
        ...
        return array($total_stock_till_date, $previous_outbound, $inbounds, $outbounds, $expired_products);
    }
