    ...
    <div v-if="page.option=='list'">
      ...
      <div class="box-body">
        ...
        <v-client-table v-model="items_in_stock" :columns="columns" :options="options">
          <div slot="total_quantity" slot-scope="{row}" class="alert alert-info">
            {{ row.total_quantity }} {{ formatPackageType(row.item.basic_unit) }}
            <!-- <br><small v-html="showItemsInCartons(row.total_quantity, row.item.total_quantity_per_carton)" /> -->
          </div>
          <div slot="total_supplied" slot-scope="{row}" class="alert alert-danger">
            {{ row.total_supplied }} {{ formatPackageType(row.item.basic_unit) }}
            <!-- <br><small v-html="showItemsInCartons((parseInt(row.total_supplied) + parseInt(row.in_transit)), row.item.total_quantity_per_carton)" /> -->
          </div>
          <div slot="total_balance" slot-scope="{row}" class="alert alert-success">
            {{ row.total_balance }} {{ formatPackageType(row.item.basic_unit) }}
            <!-- <br><small v-html="showItemsInCartons(row.total_balance - row.reserved_for_supply, row.item.total_quantity_per_carton)" /> -->
          </div>
          ...
          <div slot="action" slot-scope="props">
            ...
            <div v-if="props.row.is_warehouse_transfered === 0">
              <a v-if="checkPermission(['manage item stocks', 'delete item stocks']) && props.row.reserved_for_supply === 0 && props.row.in_transit === 0 && props.row.total_supplied === 0 && props.row.confirmed_by === null" class="btn btn-danger" @click="confirmDelete(props.index, props)"><i class="fa fa-trash" /> </a>
              <!-- <el-dropdown
...
export default {
  ...
  data() {
    return {
      ...
      items_in_stock: [],
      expired_products: [],
      columns: ['item.name', 'item.code', 'total_quantity', 'total_supplied', 'total_balance'],
      ...
      options: {
        ...
        sortable: ['item.name', 'item.code'/* 'item.name', 'batch_no', 'quantity', 'in_transit', 'supplied', 'balance', 'expiry_date', 'created_at'*/],
        filterable: ['stocker.name', 'confirmer.name', 'item.name', 'item.code'],
      },
      page: {
        option: 'list',
      },
  ...
  methods: {
    ...
    createTicket(item_stock) {
      ...
      if (new_stock_quantity > 0) {
        ...
        const details = 'Please sir,  kindly update the quantity of ' + item_stock.item.name + ' with batch no ' + item_stock.batch_no + ' from ' + item_stock.total_quantity + ' to ' + new_stock_quantity;
        ...
        const param = {
          ...
          old_quantity: item_stock.total_quantity,
        };
    ...
    fetchItemStocks() {
      ...
      itemsInStock.list(param)
        .then(response => {
          ...
          app.table_title = 'Products in ' + app.warehouses[param.warehouse_index].name;/* + ' from: ' + app.form.from + ' to: ' + app.form.to;*/
    ...
    moveExpiredProduct(index, data) {
      ...
      const param = {
        ...
        quantity: data.total_balance - data.reserved_for_supply,
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const tHeader = ['Stocked By', 'Confirmed By', 'Product', 'Batch No.', 'Quantity', 'In Transit', 'Supplied', 'Physical Stock', 'Reserved for Supply', 'Main Balance', 'Created'];
        const filterVal = ['stocker.name', 'confirmer.name', 'item.name', 'batch_no', 'quantity', 'in_transit', 'supplied', 'in_stock', 'reserved_for_supply', 'balance', 'created_at'];
