    ...
    <div v-if="page.option=='list'">
      ...
      <div class="box-header">
        ...
        <span class="pull-right">
          <a v-if="checkPermission(['manage item stocks', 'create item stocks'])" class="btn btn-info" @click="page.option = 'add_new'"> Create New</a>
        </span>
      ...
      <div class="box-body">
        ...
        <v-client-table v-model="items_in_stock" :columns="columns" :options="options">
          <div slot="quantity" slot-scope="{row}" class="alert alert-info">
            {{ row.quantity }} {{ formatPackageType(row.item.basic_unit) }}
          ...
          <div slot="supplied" slot-scope="{row}" class="alert alert-danger">
            {{ (parseInt(row.supplied) + parseInt(row.in_transit)) }} {{ formatPackageType(row.item.basic_unit) }}
          ...
          <div slot="reserved_for_supply" slot-scope="{row}" class="alert alert-default">
            <a @click="showReservationTransactions(row)">
              {{ row.reserved_for_supply }} {{ formatPackageType(row.item.basic_unit) }}
            </a>
          ...
          <div slot="in_stock" slot-scope="{row}" class="alert alert-primary">
            {{ row.balance }} {{ formatPackageType(row.item.basic_unit) }}
          ...
          <div slot="balance" slot-scope="{row}" class="alert alert-success">
            {{ (row.balance - row.reserved_for_supply) }} {{ formatPackageType(row.item.basic_unit) }}
...
export default {
  name: 'PurchaseOrder',
  components: { AddNew, EditItem, ShowItemReservationTransactions, Pagination },
  data() {
    return {
      ...
      items_in_stock: [],
      expired_products: [],
      columns: ['action', 'item.name', 'item.code', 'batch_no', 'quantity', 'supplied', 'balance', 'created_at', 'stocker.name', 'confirmer.name'],
      ...
      options: {
        ...
        sortable: ['item.name', 'batch_no'/* 'item.name', 'batch_no', 'quantity', 'in_transit', 'supplied', 'balance', 'expiry_date', 'created_at'*/],
        filterable: ['confirmer.name', 'stocker.name', 'item.name', 'item.code', 'batch_no', 'created_at'],
      },
      page: {
        option: 'list',
      },
  ...
  methods: {
    ...
    fetchItemStocks() {
      ...
      param.option = 'purchase-order';
      itemsInStock.list(param)
        .then(response => {
          app.items_in_stock = response.items_in_stock.data;
          this.items_in_stock.forEach((element, index) => {
            element['index'] = (page - 1) * limit + index + 1;
          });
          this.total = response.items_in_stock.total;
          // app.in_warehouse = 'in ' + app.warehouses[param.warehouse_index].name;
          app.table_title = `Purchase Orders  (${app.warehouses[param.warehouse_index].name})`;/* + ' from: ' + app.form.from + ' to: ' + app.form.to;*/
          loader.hide();
        })
        .catch(error => {
          loader.hide();
