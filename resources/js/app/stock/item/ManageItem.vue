    ...
    <div v-if="page.option=='list'" class="box">
      <div class="box-header">
        ...
        <span class="pull-right">
          <a v-if="canCreateNewProduct" class="btn btn-info" @click="page.option = 'add_new'"> Create New</a>
        </span>
      </div>
      <div class="box-body">
        <el-row :gutter="10">
          <el-col :xs="24" :sm="12" :md="12">
            ...
            <el-select v-model="form.category_id" placeholder="Select Product Category" class="span" filterable @input="fetchGeneralProducts">
              <el-option v-for="(category, index) in params.categories" :key="index" :value="category.id" :label="category.name" />
        ...
        <v-client-table v-model="items" :columns="columns" :options="options">
          ...
          <div slot="price.cost_price" slot-scope="{row}">
            <span align="right">{{ '₦' + Number(row.price.cost_price).toLocaleString() }}</span>
          </div>
...
export default {
  ...
  data() {
    return {
      categories: [],
      items: [],
      columns: ['action', 'name', 'code', 'category.name', 'basic_unit', 'price.cost_price', 'price.sale_price'],
      // columns: ['action', 'name', 'code', 'category.name', /* 'group_name', 'package_type' */ 'basic_unit', /* 'basic_unit_quantity_per_package_type', */ 'quantity_per_carton', 'price.cost_price', 'price.sale_price'],
      options: {
        headings: {
          name: 'Name',
          ...
          'price.cost_price': 'Cost Price',
          'price.sale_price': 'Selling Rate',
